...
interface FeatureSplitProps {
  ...
  description: string;
  features?: string[];
  subFeatures?: { title: string; desc: string; icon: string }[];
  imageSrc: string;
  overlayCard?: { title: string; value: string; label: string };
}
      ...
      <div className="container mx-auto px-6">
        <div className={`flex flex-col ${reversed ? 'md:flex-row-reverse' : 'md:flex-row'} items-center gap-16 md:gap-24`}>
          ...
          <div className="flex-1 space-y-8">
            <span className="inline-block px-4 py-1.5 bg-brand-100 text-brand-900 text-xs font-bold uppercase tracking-wider rounded-full">
              {tag}
            </span>
          ...
          <div className="flex-1 relative">
             <div className="relative rounded-[2.5rem] overflow-hidden shadow-2xl">
               ...
               {/* Overlay Overlay Card (Specific to first section) */}
               {overlayCard && (
                 <div className="absolute bottom-8 left-8 right-8 flex gap-4 animate-in slide-in-from-bottom-10 duration-700 delay-300">
               ...
               {/* Chat Overlay (Specific to second section) */}
               {subFeatures && (
                 <div className="absolute bottom-8 left-8 flex flex-col gap-4 w-64">
                   <div className="bg-white/95 backdrop-blur-sm p-3 rounded-xl shadow-lg flex items-center gap-3 animate-in slide-in-from-left-5 duration-500">
                     ...
                     <div className="flex gap-2 text-brand-500">
                       <div className="w-6 h-6 rounded-full bg-brand-100 flex items-center justify-center">📞</div>
                     </div>
                   </div>
                   <div className="bg-white/95 backdrop-blur-sm p-3 rounded-xl shadow-lg flex items-center gap-3 animate-in slide-in-from-left-5 duration-500 delay-150 ml-8">
