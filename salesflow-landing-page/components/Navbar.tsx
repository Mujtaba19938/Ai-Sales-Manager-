    ...
    >
      <div className="container mx-auto px-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-4">
          <button className="text-sm font-bold text-slate-900 hover:text-slate-700">
            Log In
          </button>
          <Button variant="black" size="sm" className="px-6">Try for Free</Button>
        </div>
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-white border-t border-slate-100 shadow-lg p-6 flex flex-col gap-4 animate-in slide-in-from-top-5 duration-200">
          {navLinks.map((link) => (
            <a 
              key={link.name} 
              href={link.href}
          ...
          <div className="flex flex-col gap-3 mt-4">
            <button className="w-full py-3 font-semibold text-slate-900 border border-slate-200 rounded-full">
              Log In
            </button>
            <Button variant="black" fullWidth>Try for Free</Button>
          </div>
        </div>
      )}
    </nav>
